 ...
 *
 * Labels: "Call pressure building" / "Put pressure building" / "Mixed flow" /
...
export function optionsPressure(contracts = [], opts = {}) {
  ...
  if (callSide) return { label: "Call pressure building", score: Math.min(100, 40 + strength), ...detail };
  if (putSide) return { label: "Put pressure building", score: Math.min(100, 40 + strength), ...detail };
...
export function pressureConfirms(label, direction) {
  if (direction === "bullish") return label === "Call pressure building";
  if (direction === "bearish") return label === "Put pressure building";
  return false;
}
