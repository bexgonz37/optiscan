...
const RULES = {
  earnings: [
    [/\b(earnings|eps|q[1-4]\s*(results|revenue)|quarterly results|beats? (estimates|expectations)|miss(es|ed)? (estimates|expectations)|(raises?|cuts?|lowers?) (guidance|outlook|forecast)|revenue (up|down|rose|fell))\b/i, 3],
  ...
  partnership: [
    [/\b(acqui(re|res|red|sition)|merger|buyout|takeover|to acquire|strategic (partnership|investment)|joint venture)\b/i, 3],
...
export function classifyCatalyst(items = [], opts = {}) {
  ...
  for (const it of items || []) {
    ...
    if (ageMs > 3 * DAY_MS || ageMs < -DAY_MS) continue; // stale or bogus timestamp
  ...
  const quality = best.strength >= 3 ? (fresh ? "strong" : "medium") : best.strength === 2 ? (fresh ? "medium" : "weak") : "weak";
