...
export function selectContract(contracts = [], opts = {}) {
  ...
  const scored = pool.map((c) => {
    // Missing greeks (common on lower data tiers) must NOT rank as a perfect
    // delta match — treat unknown delta as a mediocre 0.4 so contracts with
    // real, on-target deltas win.
    const absDelta = c.delta != null ? Math.abs(c.delta) : null;
    const deltaMatch = absDelta == null ? 0.4 : 1 - Math.min(1, Math.abs(absDelta - targetDelta) / 0.5);
