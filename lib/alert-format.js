...
import { advancedMetricsLine } from "./trade-explanation.ts";
...
export function buildScoreboardEmbed(stats = {}, rows = [], { weekly = false, dashboardUrl = "" } = {}) {
  ...
  const payload = {
    embeds: [{
      ...
      description: desc,
      fields,
      footer: { text: "Every verified callout counted, losers included" },
    }],
  };
  const safe = !containsBannedPublicLanguage(JSON.stringify(payload));
  return { payload, safe };
...
// ── Combined explanation alert (Discord) ─────────────────────────────────────
// ONE combined message. The deterministic TradeExplanation becomes the readable
// body; when a base BUY/stock embed is provided its color/author/footer/fields/
// timestamp/role-mention content are preserved (nothing critical is discarded).
// No business logic here — every string comes pre-built from the explanation.

function explanationBody(exp) {
  const lines = [];
  if (exp.whyNow) lines.push(`**Why now:** ${exp.whyNow}`);
  if (exp.contractSummary) lines.push(`**Contract:** ${exp.contractSummary}`);
  else if (exp.rejectedBecause) lines.push(`**No contract:** ${exp.rejectedBecause}`);
  if (exp.riskSummary) lines.push(`**Risk:** ${exp.riskSummary}`);
  if (exp.selectedBecause) lines.push(`**Why this contract:** ${exp.selectedBecause}`);
  if (exp.wouldImproveIf) lines.push(`**Improves if:** ${exp.wouldImproveIf}`);
  if (exp.invalidatedIf) lines.push(`**Invalidated if:** ${exp.invalidatedIf}`);
  return lines.join("\n");
}

function explanationStatusValue(exp) {
  const live = exp.actionabilityStatus === "ACTIONABLE";
  return `${exp.statusLabel}${live ? "" : " — not a live trade instruction"}`;
}
...
function upsertField(fields, name, value, inline = false) {
  ...
  if (i >= 0) fields[i] = field;
  else fields.push(field);
  return fields;
}

/**
 * Build the combined explanation Discord payload. With `base` (an existing
 * options/stock BUY embed) it merges the explanation body in and keeps the
 * base's structured metric fields as the compact advanced line + adds Status;
 * without a base it builds a standalone explanation embed.
 */
export function formatExplanationDiscord(exp, { base = null } = {}) {
  const body = explanationBody(exp);
  const advLine = advancedMetricsLine(exp);
  const statusValue = explanationStatusValue(exp);

  if (base && Array.isArray(base.embeds) && base.embeds[0]) {
    const payload = JSON.parse(JSON.stringify(base));
    const embed = payload.embeds[0];
    if (body) embed.description = body; // explanation is the readable body
    embed.fields = [...(embed.fields ?? [])];
    upsertField(embed.fields, "Advanced", advLine || "—");
    upsertField(embed.fields, "Status", statusValue);
    const safe = !containsBannedPublicLanguage(JSON.stringify(payload));
    return { payload, safe };
  }

  const color = String(exp.side ?? "").toLowerCase().startsWith("p")
    ? DISCORD_COLORS.put
    : exp.side === "call"
      ? DISCORD_COLORS.call
      : DISCORD_COLORS.neutral;
  const fields = [];
  upsertField(fields, "Advanced", advLine || "—");
  upsertField(fields, "Status", statusValue);
  const payload = {
    embeds: [{
      color,
      author: { name: "OPTISCAN · options" },
      title: exp.plainSummary,
      description: body,
      fields,
      footer: { text: "research signal, not financial advice" },
      timestamp: new Date().toISOString(),
    }],
  };
  const safe = !containsBannedPublicLanguage(JSON.stringify(payload));
