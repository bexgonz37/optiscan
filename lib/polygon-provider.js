...
export function parseSnapshotTickers(tickers) {
  ...
  for (const t of tickers) {
    ...
    const day = t.day || {};
    const min = t.min || {};
    const last = numOrNull(t.lastTrade?.p) ?? numOrNull(min.c) ?? numOrNull(day.c);
    out.push({
...
export async function fetchBulkQuotes(symbols = []) {
  ...
  try {
    ...
    return { available: true, quotes: parseSnapshotTickers(raw.tickers), source: "polygon" };
  } catch (err) {
...
export async function fetchTopMovers(direction = "gainers", limit = 20) {
  ...
  try {
    ...
    return { available: true, quotes: parseSnapshotTickers(raw.tickers).slice(0, limit), source: "polygon" };
  } catch (err) {
    return { available: false, quotes: [], note: err.message, source: "polygon" };
  }
}
...
export async function fetchCandles(symbol, opts = {}) {
  ...
  try {
    const raw = await polyRequest(`/v2/aggs/ticker/${encodeURIComponent(sym)}/range/${mult}/${timespan}/${from}/${to}`, {
      adjusted: "true",
      sort: "asc",
...
export async function fetchOptionChain(underlying, opts = {}) {
  ...
  try {
    const raw = await polyRequest(`/v3/snapshot/options/${encodeURIComponent(sym)}`, params);
