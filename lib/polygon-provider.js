...
async function polyFetch(url) {
  ...
  account("ok", {
    latencyMs,
    recordsReturned: Array.isArray(json?.results) ? json.results.length : null,
 ...
 *
 * @returns {Promise<{available:boolean, contract:object|null, quotaExceeded:boolean, note?:string, source:string}>}
 */
export async function fetchOptionContractSnapshot(underlying, optionSymbol) {
  ...
  try {
    ...
    if (!contract) {
      ...
      return { available: true, contract: null, quotaExceeded: false, note: `no snapshot returned for ${occ}`, source: "polygon" };
    }
    recordDataSample({ symbol: sym, kind: "options_quote", providerTimestamp: contract.providerTimestamp });
    return { available: true, contract, quotaExceeded: false, source: "polygon" };
  } catch (err) {
    ...
    return { available: false, contract: null, quotaExceeded, note: err.message, source: "polygon" };
  }
}
