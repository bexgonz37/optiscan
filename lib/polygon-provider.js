...
async function polyFetch(url) {
  ...
  /**
   * `atMs` decides which MINUTE BUCKET a request lands in, and the budget is
   * enforced per minute at ISSUE time. Accounting a completed request at
   * RESPONSE time therefore filed slow answers under a minute that never
   * spent them: production reported a peak of 449 requests in one minute
   * against a 280 cap — `capUtilizationPctAtPeak: 160.4%` — with whole minutes
   * missing entirely, because a 70-second stall moved every request issued
   * during it into a later bucket. No cap was breached; the dashboard an
   * operator reads to answer "who saturated the minute" was simply not
   * bucketing by the clock the cap uses. Pass the issue instant explicitly.
   */
  let issuedAtMs = Date.now();
  const account = (status, extra = {}) => {
    try {
      emitProviderRequest({ endpoint, status, atMs: issuedAtMs, ...extra });
  ...
  try {
    ...
    // never confused with missing market data. A refusal is instantaneous, so its
    // issue instant and its outcome instant are the same moment.
    account("quota_block");
    throw err;
  }
