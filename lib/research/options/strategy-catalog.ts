 ...
 * NOT actionable by itself and does not send alerts. Bearish/PUT delivery is
 * governed by lib/research/options/bearish-authority.ts.
 */
...
export interface StrategyDef {
  ...
  /**
   * Which symbols this strategy is written for. "index" means it is only meaningful on
   * the index ETFs (its IDX_LIQ/IDX_OPT thresholds are unreachable elsewhere anyway);
   * absent means any qualifying underlying.
   *
   * This is also the tie-break discriminator that makes the index strategies reachable.
   * Selection takes the single highest-scoring strategy, and ties previously resolved by
   * CATALOG ARRAY POSITION, which is arbitrary and invisible — see selectOptionsStrategy.
   */
  symbolScope?: "index";
}
...
export const OPTIONS_STRATEGIES: StrategyDef[] = [
  ...
  { key: "momentum_breakdown", label: "Momentum breakdown", side: "put", entryTrigger: "support breaks with downside acceleration and volume", earlySignals: ["lod_break", "downside_acceleration", "below_vwap", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: { minOpenInterest: 1000, minContractVolume: 1000, maxSpreadPct: 4 }, preferredDte: ["0dte", "1-7dte"], preferredDelta: [0.40, 0.60], moneyness: "ATM", freshnessMaxMs: 10_000, chaseLimitPct: 0.45, stop: "reclaim of breakdown level / VWAP", targets: "1R fast / next support", holdingHorizon: "intraday", sessions: ["regular"], grading: "real_option_pl" },
  { key: "failed_breakout_reversal", label: "Failed breakout reversal", side: "put", entryTrigger: "failed high break rejects and rotates back under VWAP", earlySignals: ["hod_break", "below_vwap", "downside_acceleration", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: { minOpenInterest: 1000, minContractVolume: 750, maxSpreadPct: 5 }, preferredDte: ["0dte", "1-7dte"], preferredDelta: [0.40, 0.60], moneyness: "ATM", freshnessMaxMs: 15_000, chaseLimitPct: 0.45, stop: "reclaim of failed breakout high", targets: "range low / next support", holdingHorizon: "intraday", sessions: ["regular"], grading: "real_option_pl" },
  { key: "vwap_rejection", label: "VWAP rejection", side: "put", entryTrigger: "failed VWAP reclaim with sellers accelerating", earlySignals: ["below_vwap", "downside_momentum", "downside_acceleration", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: { minOpenInterest: 1000, minContractVolume: 750, maxSpreadPct: 4 }, preferredDte: ["0dte", "1-7dte"], preferredDelta: [0.40, 0.60], moneyness: "ATM", freshnessMaxMs: 10_000, chaseLimitPct: 0.5, stop: "clean reclaim and hold above VWAP", targets: "LOD retest / extension", holdingHorizon: "intraday", sessions: ["regular"], grading: "real_option_pl" },
  { key: "support_break_retest", label: "Support break and retest", side: "put", entryTrigger: "support breaks, retest fails, spread stays tight", earlySignals: ["support_break_proximity", "lod_break", "below_vwap", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: { minOpenInterest: 1000, minContractVolume: 750, maxSpreadPct: 5 }, preferredDte: ["0dte", "1-7dte", "8-14dte"], preferredDelta: [0.35, 0.60], moneyness: "ATM_OTM", freshnessMaxMs: 15_000, chaseLimitPct: 0.5, stop: "reclaim of broken support", targets: "next support / measured breakdown", holdingHorizon: "intraday-2 days", sessions: ["regular"], grading: "real_option_pl" },
  { key: "lower_high_continuation", label: "Lower-high continuation", side: "put", entryTrigger: "lower high forms below VWAP with renewed downside momentum", earlySignals: ["below_vwap", "downside_momentum", "compression_near_level"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: OPT_LIQ, preferredDte: ["1-7dte", "8-14dte"], preferredDelta: [0.35, 0.55], moneyness: "ATM_OTM", freshnessMaxMs: 20_000, chaseLimitPct: 0.45, stop: "break of lower-high structure", targets: "prior low / trend extension", holdingHorizon: "hours-2 days", sessions: ["regular"], grading: "real_option_pl" },
  { key: "bearish_opening_range_break", label: "Bearish opening-range break", side: "put", entryTrigger: "opening range low breaks with sell-volume confirmation", earlySignals: ["opening_range_development", "lod_break", "downside_acceleration", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: { minOpenInterest: 1000, minContractVolume: 1000, maxSpreadPct: 4 }, preferredDte: ["0dte", "1-7dte"], preferredDelta: [0.40, 0.60], moneyness: "ATM", freshnessMaxMs: 10_000, chaseLimitPct: 0.45, stop: "reclaim of opening range low", targets: "OR width extension", holdingHorizon: "intraday", sessions: ["regular"], grading: "real_option_pl" },
  { key: "gap_failure", label: "Gap failure", side: "put", entryTrigger: "gap up fails/reverses or gap down continues below VWAP", earlySignals: ["earnings_gap", "below_vwap", "downside_momentum", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: OPT_LIQ, preferredDte: ["1-7dte", "8-14dte"], preferredDelta: [0.35, 0.55], moneyness: "ATM_OTM", freshnessMaxMs: 30_000, chaseLimitPct: 0.45, stop: "reclaim of gap failure pivot", targets: "gap fill / next support", holdingHorizon: "hours-3 days", sessions: ["regular"], grading: "real_option_pl" },
  { key: "relative_weakness_continuation", label: "Relative-weakness continuation", side: "put", entryTrigger: "ticker weak versus market with bearish structure confirmed", earlySignals: ["below_vwap", "downside_momentum", "put_flow_skew", "rel_volume"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: OPT_LIQ, preferredDte: ["1-7dte", "8-14dte", "15-30dte"], preferredDelta: [0.35, 0.55], moneyness: "ATM_OTM", freshnessMaxMs: 30_000, chaseLimitPct: 0.45, stop: "relative weakness resolves / VWAP reclaim", targets: "trend extension", holdingHorizon: "hours-5 days", sessions: ["regular"], grading: "real_option_pl" },
  { key: "downside_catalyst_continuation", label: "Downside catalyst continuation", side: "put", entryTrigger: "bearish catalyst plus confirmed downside continuation", earlySignals: ["earnings_timing", "earnings_gap", "downside_momentum", "below_vwap", "put_flow_skew"], underlyingLiquidity: CORE_LIQ, optionsLiquidity: OPT_LIQ, preferredDte: ["1-7dte", "8-14dte", "15-30dte"], preferredDelta: [0.35, 0.55], moneyness: "ATM_OTM", freshnessMaxMs: 60_000, chaseLimitPct: 0.4, stop: "catalyst thesis invalidated / VWAP reclaim", targets: "catalyst measured move", holdingHorizon: "1-5 days", sessions: ["regular"], grading: "real_option_pl" },
