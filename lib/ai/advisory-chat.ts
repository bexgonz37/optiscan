...
function systemPrompt(packet: EvidencePacket, mode: ChatMode): string {
  const lines = [
    ...
    "",
    "ABSOLUTE RULES:",
    "1. You may ONLY state numbers that appear in the EVIDENCE list below. Never compute, estimate, round differently, or infer a number that is not there.",
    "2. Cite the evidence id for every numeric claim, in the citedEvidenceIds array.",
    "3. Never combine metrics from different pipelines or different time windows in one claim.",
