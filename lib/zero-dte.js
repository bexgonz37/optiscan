...
// Contract SELECTION/GATING is consolidated in lib/contract-selector.ts. The
// three exports below (rankZeroDteContracts, contractEntryGate, nearTheMoneyPair)
// are thin compatibility wrappers that delegate to the canonical implementations
// so callers and tests keep their exact signatures. (Lazy call-time bindings —
// the selector reuses zeroDteContractScore from here; no init-time cycle.)
import { rankZeroDte, entryGate, nearTheMoney } from "./contract-selector.ts";
 ...
 *
 * @param {{ status?: string, contractScore?: number|null, minsToClose?: number|null,
 *           spreadPct?: number|null, efficiency?: number|null, ivPremium?: number|null }} input
 */
export function optionStillWorthIt({ status, contractScore, minsToClose, spreadPct, efficiency, ivPremium = null }) {
  ...
  // IV premium = contractIV / realizedVol. Rich options need to be right and fast.
  let ivPart = 0;
  if (isNum(ivPremium)) {
    if (ivPremium < 0.85) ivPart = 8;
    else if (ivPremium < 1.25) ivPart = 4;
    else if (ivPremium < 1.75) ivPart = -6;
    else ivPart = -14;
  }
  const score = Math.round(clamp(statusPart + contractPart + timePart + spreadPart + effPart + ivPart, 0, 100));

  let verdict;
  if (isNum(efficiency) && efficiency < 0.3) verdict = "Too Choppy / Skip";
  else if (status === "exhausted") verdict = "Too Late / Skip";
  else if (isNum(ivPremium) && ivPremium >= 1.75) verdict = "IV Rich / Fast Only";
...
export function shouldTrigger({
  ...
  minRate = 0.2, minSurge = 1.4, minLevelSurge = 1.2, minEfficiency = 0.35,
}) {
...
export function speedPersistentFromRing(ring, {
  minRate = 0.2, direction = "bullish", window = 5, minHits = 3, subWindowMs = 4000, nowMs,
} = {}) {
...
/**
 * HARD entry-economics gate for the TRADE tier — the audit's graded order
 * (9.2% spread, breakeven 3.4% of spot) shows why score points aren't enough:
 * a +15% mid winner is a real-fill loser after a 9% spread round trip.
 * Research/WAIT alerts still persist; only the BUY callout requires this.
 */
/**
 * @param {any} contract
 * @param {{ underlying?: number|null, expRemainPct?: number|null, maxSpreadPct?: number, minDelta?: number, maxDelta?: number }} [opts]
 * @returns {{ ok: boolean, failures: string[] }}
 */
/** Thin wrapper → the canonical 0DTE tradability gate in contract-selector.ts. */
export function contractEntryGate(contract, opts = {}) {
  return entryGate(contract, opts);
}

/**
 * Trend-alignment gate for the TRADE tier. Audit failure modes it blocks:
 *   - counter-trend flickers (CALL on a -6.8% day, PUT on a +1.25% day)
 *   - late chasing (side aligned but the day move is already exhausted:
 *     PUTs fired at -7…-14% mean-reverted at 5m)
 * An aligned fresh HOD/LOD break is the only exception on extended names.
 */
/**
 * @param {{ direction?: string|null, movePct?: number|null, hodBreak?: boolean|null, lodBreak?: boolean|null, maxCounterPct?: number, maxExtendedPct?: number }} [input]
 * @returns {{ ok: boolean, why: string|null }}
 */
export function trendAlignedForTrade({ direction = null, movePct = null, hodBreak = false, lodBreak = false, maxCounterPct = 1.5, maxExtendedPct = 6 } = {}) {
  const move = isNum(movePct) ? movePct : 0;
  const dirUp = direction !== "bearish";
  const alignedBreak = dirUp ? Boolean(hodBreak) : Boolean(lodBreak);
  if (dirUp && move < -maxCounterPct && !alignedBreak) return { ok: false, why: `call against a ${move.toFixed(1)}% down day` };
  if (!dirUp && move > maxCounterPct && !alignedBreak) return { ok: false, why: `put against a +${move.toFixed(1)}% up day` };
  if (Math.abs(move) > maxExtendedPct && !alignedBreak) return { ok: false, why: `day move ${move.toFixed(1)}% already extended — needs a fresh level break` };
  return { ok: true, why: null };
}

/**
 * Rank one side's 0DTE contracts. User trades close-to-spot strikes: rank by
 * (1) delta in the usable 0.35-0.65 zone, (2) strike distance from spot,
 * (3) composite quality score. Never "cheapest premium wins". When
 * `underlying` is not provided, falls back to pure score order.
 */
export function rankZeroDteContracts(contracts = [], side, opts = {}) {
  return rankZeroDte(contracts, side, opts);
}

/**
 * Near-the-money pair for display: the closest usable CALL and PUT to spot
 * (delta 0.35-0.65 preferred, 0.25-0.75 fallback) with live entry economics —
 * bid/ask/mid/spread and "needs X% move to breakeven".
 */
/**
 * @param {any[]} contracts
 * @param {number|null} spot
 * @param {{ expRemainPct?: number|null }} [opts]
 */
export function nearTheMoneyPair(contracts = [], spot = null, opts = {}) {
  return nearTheMoney(contracts, spot, opts);
}
