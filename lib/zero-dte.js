 ...
 */
export function shouldTrigger({ shortRate, surge, hodBreak, lodBreak, efficiency, nowMs, cooldownUntil, minRate = 0.15, minSurge = 1.3 }) {
  ...
  if (isNum(efficiency) && efficiency < 0.3) return false; // too choppy
  const volumeConfirms = surge != null && surge >= minSurge;
  const levelBreak = Boolean(hodBreak || lodBreak);
