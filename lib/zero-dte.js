 ...
 */
export function contractEntryGate(contract, {
  underlying = null, expRemainPct = null,
  maxSpreadPct = 5, minDelta = 0.35, maxDelta = 0.65,
} = {}) {
  const failures = [];
 ...
 */
export function rankZeroDteContracts(contracts = [], side, { minsToClose, expRemainPct, max = 3, underlying = null } = {}) {
  const spot = isNum(underlying) ? underlying : null;
  const inZone = (c) => {
    const d = isNum(c.delta) ? Math.abs(c.delta) : null;
    return d != null && d >= 0.35 && d <= 0.65;
  };
  const dist = (c) => (spot != null && isNum(c.strike) ? Math.abs(c.strike - spot) / spot : Infinity);
  return contracts
 ...
 */
export function nearTheMoneyPair(contracts = [], spot = null, { expRemainPct = null } = {}) {
  const pick = (side) => {
    if (!isNum(spot) || spot <= 0) return null;
    const usable = contracts.filter((c) => c.side === side && isNum(c.mid) && c.mid > 0 && isNum(c.strike));
    const zone = (lo, hi) => usable.filter((c) => {
      const d = isNum(c.delta) ? Math.abs(c.delta) : null;
