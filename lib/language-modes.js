...
// ── UI directive labels (audit P0-4/T5) ─────────────────────────────────────
// The web UI must honor language mode too, not just Discord. In public /
// education mode no screen may print directive trading language ("BUY CALL",
// "LONG", a literal order line). Components read these maps via
// uiDirectiveLabel() / publicizeDirectiveText() and the useLanguageMode hook.

export const UI_DIRECTIVE_LABELS = {
  private: {
    buy_call: "BUY CALL",
    buy_put: "BUY PUT",
    long: "LONG",
  ...
  public: {
    buy_call: "Call Momentum Watch",
    buy_put: "Put Momentum Watch",
    long: "Bullish Share Momentum Watch",
    short: "Bearish Share Momentum Watch",
    trade_tier: "High-conviction watch",
  },
};

/** Mode-aware label for a directive concept. Unknown kind returns "". */
export function uiDirectiveLabel(kind, mode = "private") {
  const set = UI_DIRECTIVE_LABELS[mode === "public" ? "public" : "private"];
  return set[kind] ?? "";
}

/**
 * Rewrite known directive phrases in a headline/sentence for public mode.
 * Backstop: if the result still fails the banned-language guard, collapse to
 * a neutral label — public output must NEVER leak a directive.
 */
export function publicizeDirectiveText(text) {
  let t = String(text ?? "");
  const swaps = [
    [/BUY CALL\s*\/\s*(BUY )?PUT/gi, "Call/Put Momentum Watch"],
    [/BUY CALL/gi, "Call Momentum Watch"],
    [/BUY PUT/gi, "Put Momentum Watch"],
    [/Buy stock \u2191/gi, "Bullish Share Momentum Watch"],
    [/Bet stock \u2193/gi, "Bearish Share Momentum Watch"],
    [/Buy call option/gi, "Call momentum"],
    [/Buy put option/gi, "Put momentum"],
    [/\bLONG\s*\/\s*SHORT\b/g, "bullish/bearish"],
    [/when TRADE fires?/gi, "when a high-conviction signal fires"],
    [/\bwhen TRADE\b/gi, "when a high-conviction signal shows"],
  ];
  for (const [re, sub] of swaps) t = t.replace(re, sub);
  if (containsBannedPublicLanguage(t)) return "Momentum Watch";
  return t;
}

/** Order-ticket UI (copy "BUY 1 ..." lines) is private-mode only. */
export function showOrderTicket(mode) {
  return mode !== "public";
}

/** Standing footer shown whenever the UI is in public / education mode. */
export const PUBLIC_MODE_DISCLAIMER =
  "OptiScan is a momentum scanner for education. Nothing here is financial advice or a recommendation to trade. OptiScan is not a broker and never places trades.";
