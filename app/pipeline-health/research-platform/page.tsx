...
/**
 * /research ΓÇö READ-ONLY operational view of the multi-lane research rebuild (Phase 8).
 * Summary cards first, drill-down below. Inactive/missing-provider states are shown
 * honestly (never a misleading "healthy" green). No write controls. All paper ΓÇö never
...
export default function ResearchOverviewPage() {
  ...
  if (loading) return <main style={{ padding: 24, fontFamily: "system-ui, sans-serif" }}>Loading research overviewΓÇª</main>;
  ...
  const Section = ({ title, obj }: { title: string; obj: any }) => (
    <details style={{ margin: "8px 0", border: "1px solid #e5e7eb", borderRadius: 8 }}>
      <summary style={{ cursor: "pointer", padding: "10px 14px", fontWeight: 600 }}>{title}{obj?.error ? " ΓÜá∩╕Å" : ""}</summary>
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui, sans-serif", maxWidth: 1100, margin: "0 auto" }}>
      <h1 style={{ marginBottom: 4 }}>Research Platform ΓÇö Operational View</h1>
      ...
      {funnel?.available === false ? (
        <p style={{ color: "#6b7280" }}>No candidates captured yet ΓÇö {funnel.note}.</p>
      ) : (
      ...
      <h2 style={{ fontSize: 16, marginTop: 24 }}>Portfolios (independent ΓÇö not mirrors)</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
          {Object.entries(ports).map(([name, p]: [string, any]) => (
            <div key={name} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12 }}>
              ...
              <div style={{ fontSize: 13, marginTop: 6 }}>equity ${p.equity} ┬╖ closed {p.closedTrades} ┬╖ win {p.winRatePct ?? "ΓÇö"}%</div>
