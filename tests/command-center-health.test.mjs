...
test("PRODUCT nav matches decision-first destinations", () => {
  ...
  assert.ok(/label: "AI OPTIONS"/.test(shell));
  assert.ok(/label: "QUANT"/.test(shell));
  assert.ok(/label: "WATCHLIST"/.test(shell));
  assert.ok(/label: "DISCORD"/.test(shell));
  assert.ok(/label: "PAPER"/.test(shell));
  assert.ok(/label: "SETTINGS"/.test(shell));
  assert.ok(/label: "MORE"/.test(shell));
  assert.ok(/Alerts History/.test(shell));
