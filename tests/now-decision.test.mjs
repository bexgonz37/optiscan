...
test("owner research test route exists and test sender skips idempotency log", () => {
  const route = read("app/api/research/owner-research/test-notification/route.ts");
  assert.match(route, /sendOwnerResearchTestNotification/);
  assert.match(route, /stateChanged: false/);
  const notify = read("lib/notifications/owner-research-notify.ts");
  assert.match(notify, /sendOwnerResearchTestNotification/);
  assert.match(notify, /OWNER RESEARCH NOTIFICATION/);
  assert.match(notify, /VERIFY CONTRACT AFTER OPTIONS OPEN/);
