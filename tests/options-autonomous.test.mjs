...
function db() {
  ...
  d.exec(`CREATE TABLE options_candidates (id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT NOT NULL, tier INTEGER, session TEXT, selected_strategy TEXT, direction TEXT, side TEXT, research_only INTEGER NOT NULL DEFAULT 0, score REAL, considered_json TEXT, state TEXT NOT NULL, why TEXT, option_symbol TEXT, chain_fetch_ms INTEGER, freshness_state TEXT, callout_message TEXT, latency_json TEXT, earliness_phase TEXT, escalated_by TEXT, feature_snapshot_json TEXT, created_at_ms INTEGER NOT NULL);
          ...
          CREATE TABLE options_paper_marks (id INTEGER PRIMARY KEY AUTOINCREMENT, trade_id INTEGER NOT NULL, option_symbol TEXT NOT NULL, mark_at_ms INTEGER NOT NULL, bid REAL, ask REAL, exit_fill REAL, return_pct REAL, quote_age_ms INTEGER, created_at_ms INTEGER NOT NULL, UNIQUE(trade_id, mark_at_ms));
...
test("2. scanning and grading run in-process — no HTTP/endpoint call required", async () => {
  ...
  assert.ok(mark.mfe_pct > 0, "fresh grading marks update observed MFE");
  assert.equal(mark.mae_pct, mark.mfe_pct, "single observed mark is both MFE and MAE until more marks exist");
  assert.equal(d.prepare("SELECT COUNT(*) n FROM options_paper_marks WHERE trade_id=?").get(id).n, 1);
});
...
test("5. dedup is DB-based → survives a restart (no duplicate alert, no duplicate paper trade)", async () => {
  ...
  const input = { candidateSymbol: "NVDA", strategy: "momentum_acceleration", researchOnly: false, contract: { optionSymbol: "O:NVDA260117C00100000", side: "call", strike: 100, expiration: "2026-01-17", dte: 5, bid: 1.0, ask: 1.1, spreadPct: 5, quoteAgeMs: 1000, volume: 1000, openInterest: 5000, delta: 0.5, providerTimestamp: NOW - 1000 }, message: "buy", observedUnderlyingPrice: 100, currentUnderlyingPrice: 100, chaseLimitPct: 5, underlyingPrice: 100 };
