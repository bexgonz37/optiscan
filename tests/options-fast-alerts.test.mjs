...
function db() {
  ...
  d.exec(`CREATE TABLE options_candidates (id INTEGER PRIMARY KEY AUTOINCREMENT, symbol TEXT NOT NULL, tier INTEGER, session TEXT, selected_strategy TEXT, direction TEXT, side TEXT, research_only INTEGER NOT NULL DEFAULT 0, score REAL, considered_json TEXT, state TEXT NOT NULL, why TEXT, option_symbol TEXT, chain_fetch_ms INTEGER, freshness_state TEXT, callout_message TEXT, latency_json TEXT, earliness_phase TEXT, escalated_by TEXT, feature_snapshot_json TEXT, created_at_ms INTEGER NOT NULL);
          ...
          CREATE UNIQUE INDEX options_paper_one_active_thesis_idx ON options_paper_trades(thesis_fingerprint) WHERE status IN ('PENDING_DELIVERY','ENTERED') AND thesis_fingerprint IS NOT NULL;
...
const monDeps = (d) => ({ now: () => NOW, session: () => "regular", getDb: () => d, getUnderlyingBatch: async (s) => snap(s), getChain: async () => chainOk([]) });
...
test("opening window: 20 simultaneous broad candidates produce at most the rolling-window limit", async () => {
  ...
  const mk = (sym, quoteNow = openingNow) => ({ candidateSymbol: sym, strategy: "momentum_acceleration", researchOnly: false, contract: { optionSymbol: `O:${sym}260724C00100000`, side: "call", strike: 100, expiration: "2026-07-24", dte: 2, bid: 1.0, ask: 1.1, spreadPct: 5, quoteAgeMs: 1000, volume: 1000, openInterest: 5000, delta: 0.5, providerTimestamp: quoteNow - 1000 }, message: "x", observedUnderlyingPrice: 100, currentUnderlyingPrice: 100, chaseLimitPct: 5, underlyingPrice: 100, decisionMs: quoteNow, tier: 2 });
  for (let i = 0; i < 20; i++) await deliverOptionsCallout(mk(`B${String.fromCharCode(65 + i)}`), { getDb: () => d, send, now: () => openingNow }, ENV);
...
test("opening window: Tier 0 is EXEMPT from the cap so SPY beats broad opening noise", async () => {
  ...
  let sends = 0;
  const send = async () => { sends += 1; return { ok: true, status: 204, messageId: "m", latencyMs: 3, ambiguous: false, error: null }; };
  const mk = (sym, tier) => ({ candidateSymbol: sym, strategy: "momentum_acceleration", researchOnly: false, contract: { optionSymbol: `O:${sym}260724C00100000`, side: "call", strike: 100, expiration: "2026-07-24", dte: 2, bid: 1.0, ask: 1.1, spreadPct: 5, quoteAgeMs: 1000, volume: 1000, openInterest: 5000, delta: 0.5, providerTimestamp: openingNow - 1000 }, message: "x", observedUnderlyingPrice: 100, currentUnderlyingPrice: 100, chaseLimitPct: 5, underlyingPrice: 100, decisionMs: openingNow, tier });
  for (let i = 0; i < 5; i++) await deliverOptionsCallout(mk(`B${String.fromCharCode(65 + i)}`, 2), { getDb: () => d, send, now: () => openingNow }, ENV);
...
test("same symbol+side+strategy within the window is suppressed; a NEW strategy is a new setup", async () => {
  ...
  let sends = 0;
  const send = async () => { sends += 1; return { ok: true, status: 204, messageId: "m", latencyMs: 3, ambiguous: false, error: null }; };
  const mk = (strategy, strike, quoteNow = t) => ({ candidateSymbol: "NVDA", strategy, researchOnly: false, contract: { optionSymbol: `O:NVDA260724C00${strike}000`, side: "call", strike, expiration: "2026-07-24", dte: 2, bid: 1.0, ask: 1.1, spreadPct: 5, quoteAgeMs: 1000, volume: 1000, openInterest: 5000, delta: 0.5, providerTimestamp: quoteNow - 1000 }, message: "x", observedUnderlyingPrice: 100, currentUnderlyingPrice: 100, chaseLimitPct: 5, underlyingPrice: 100, decisionMs: quoteNow, tier: 1 });
