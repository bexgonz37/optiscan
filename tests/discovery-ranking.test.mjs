...
test("fresh fast mover outranks a slow grinder that is already up more", () => {
  const prev = new Map([
    ["FAST", prevAt(1.0, 30)],  // was +1.0%, now +3.0% → +2%/min fresh accel
    ["SLOW", prevAt(5.4, 30)],  // was +5.4%, now +5.5% → ~0.2%/min, already extended-ish
  ]);
  const quotes = [
    { symbol: "FAST", price: 20, changePercent: 3.0, volume: 2_000_000 },
    { symbol: "SLOW", price: 20, changePercent: 5.5, volume: 2_000_000 },
  ];
...
test("exceptional fresh mover is flagged for immediate promotion", () => {
  const prev = new Map([["ROCKET", prevAt(0.5, 20)]]); // +0.5%→+2.0% in 20s = 4.5%/min
  const ranked = rankDiscovery([{ symbol: "ROCKET", price: 15, changePercent: 2.0, volume: 500_000 }], prev, NOW, cfg);
...
test("a reversal (giving back gains) gets no fresh-accel boost", () => {
  const prev = new Map([["FADE", prevAt(4.0, 30)]]); // was +4%, now +2% → negative velocity
  const ranked = rankDiscovery([{ symbol: "FADE", price: 15, changePercent: 2.0, volume: 500_000 }], prev, NOW, cfg);
...
test("a fresh mover beats an extended-and-slow one (not up-a-lot wins)", () => {
  const prev = new Map([
    ["FRESH", prevAt(2.0, 30)],  // +2%→+3% = fresh +2%/min
    ["EXTSLOW", prevAt(11.9, 30)], // +12% day but only +0.2%/min and extended
  ]);
  const ranked = rankDiscovery([
    { symbol: "FRESH", price: 20, changePercent: 3.0, volume: 1_000_000 },
    { symbol: "EXTSLOW", price: 20, changePercent: 12.0, volume: 1_000_000 },
  ], prev, NOW, cfg);
  assert.ok(
...
test("below-min-volume and non-positive price are filtered out", () => {
  const ranked = rankDiscovery([
    { symbol: "THIN", price: 20, changePercent: 5, volume: 100 },
    { symbol: "ZERO", price: 0, changePercent: 5, volume: 5_000_000 },
    { symbol: "OK", price: 20, changePercent: 5, volume: 5_000_000 },
  ], new Map(), NOW, cfg);
...
test("promotionSet includes an immediate-promote name beyond topN", () => {
  ...
  const prev = new Map([["ROCKET", prevAt(0.2, 20)]]);
  const quotes = [
    { symbol: "BIG", price: 20, changePercent: 5.9, volume: 9_000_000 },   // ranks #1 on volume+move
    { symbol: "MID", price: 20, changePercent: 3.0, volume: 8_000_000 },
    { symbol: "ROCKET", price: 20, changePercent: 1.7, volume: 200_000 },  // fresh accel → immediate
  ];
