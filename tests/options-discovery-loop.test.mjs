...
test("evaluateCallout emits ONE READY compact callout: WATCHING header + approx entry + targets, no published stop", () => {
  ...
  assert.match(r.message, /^\*\*WATCHING HOOD \$42 CALL\*\* - exp 03\/20$/m);
  assert.match(r.message, /^Entry around \$1\.25 \| Targets \$\d+\.\d{2} \/ \$\d+\.\d{2}$/m);
  assert.doesNotMatch(r.message, /n\/a|Why:|Stop:/, "no n/a targets, no Why:, no published stop");
