    ...
    <div className="help-section">
      <div className="guide-intro panel main">
        ...
        <p className="muted settings-desc">
          ...
          {isPublic ? " bullish/bearish watches" : " LONG/SHORT callouts"} across premarket, regular hours, and after-hours. You always make the final
      ...
      <Section title="2. Market hours — options">
        <ul className="guide-list">
          <li><strong>{isPublic ? "Call momentum ↑" : "Buy call option ↑"}</strong> — stock moving UP fast with a liquid same-day call.</li>
          <li><strong>{isPublic ? "Put momentum ↓" : "Buy put option ↓"}</strong> — stock moving DOWN fast with a liquid same-day put.</li>
      ...
      <Section title="3. Market momentum — shares only">
        <ul className="guide-list">
          <li><strong>{isPublic ? "Bullish shares ↑" : "Buy stock ↑"}</strong> / <strong>{isPublic ? "Bearish shares ↓" : "Bet stock ↓"}</strong> — share signals only, no options.</li>
          ...
          <li>{isPublic ? "Stock signals appear" : "Stock BUYs appear"} in the Market hero and the dedicated stock Discord channel; browser popups remain options-only.</li>
        </ul>
      </Section>

      <Section title="4. Live page">
        <ul className="guide-list">
          <li><strong>Options</strong> — 0DTE hero, ticket, and options history. <strong>Market</strong> — shares-only {isPublic ? "bullish/bearish" : "LONG/SHORT"} hero and history.</li>
        </ul>
      </Section>

      <Section title="5. Alerts page">
        <ul className="guide-list">
          <li><strong>Right now</strong> — {isPublic ? "live high-conviction signals" : "live buy signals"}. <strong>History</strong> — past alerts + track record. <strong>Journal</strong> — your log.</li>
        </ul>
      </Section>
    </div>
  );
